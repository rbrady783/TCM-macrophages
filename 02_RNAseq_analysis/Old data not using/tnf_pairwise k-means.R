...
raw_tnfa_k <- select(raw, 1,3,4,6:10,13:19,21,22,24,25)

# 3. Read the sample metadata from tnfa_groups.csv
tnfa_groups_k <- read.csv('tnfa_groups_k.csv', header = TRUE, stringsAsFactors = FALSE)

# Set the row names to the gene IDs (first column), then remove that column.
rownames(raw_tnfa_k) <- raw_tnfa_k[[1]]
raw_tnfa_k <- raw_tnfa_k[, -1]

# Check that every column name in raw_tnfa (i.e. sample IDs) is found in tnfa_groups$id
if (all(colnames(raw_tnfa_k) %in% tnfa_groups_k$id)) {
  cat("All sample names in raw_ are present in _groups$id.\n")
} else {
  missing_cols <- setdiff(colnames(raw_tnfa_k), tnfa_groups_k$id)
...
tnfa_groups_k$group <- factor(tnfa_groups_k$group)
tnfa_groups_k$group <- relevel(tnfa_groups_k$group, ref = "low")
...
dds_tnf_k <- DESeqDataSetFromMatrix(countData = raw_tnfa_k,
                                  colData = tnfa_groups_k,
                                  design = ~ group)

# Run the DESeq2 analysis (this will estimate size factors and perform normalization internally)
dds_tnf_k <- DESeq(dds_tnf_k)

# Retrieve the results (differential expression statistics)
res_tnf_k <- results(dds_tnf_k)
head(as.data.frame(res_tnf_k))
summary(res_tnf_k)

# Save the results to a CSV file
output_fn <- "C:/Users/brady/OneDrive/Desktop/TCM_RNA/Output/CorrelationTables/tnf_pairwise_k.csv"
...
with(as.data.frame(res_tnf_k), plot(log2FoldChange, -log10(padj),
                                  pch = 20, main = "TNF-a (k means)"))

# Highlight significant points consistently using padj
with(subset(as.data.frame(res_tnf_k), padj < 0.01 & log2FoldChange >= 2), 
