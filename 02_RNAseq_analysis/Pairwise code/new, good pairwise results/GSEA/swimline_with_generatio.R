...
format_pathway_names <- function(pathway_names) {
  ...
  # Remove "Overview of" prefix - use ignore.case to catch all variations
  t <- gsub("^Overview of ", "", t, ignore.case = TRUE)
  t <- trimws(t)  # Clean up any extra whitespace
...
# Order pathways by AVERAGE gene ratio
pathway_gene_ratio <- df %>%
  group_by(pathway_clean) %>%
  summarise(sort_gene_ratio = mean(gene_ratio, na.rm = TRUE)) %>%
  arrange(sort_gene_ratio)

pathway_order_levels <- pathway_gene_ratio$pathway_clean
...
# Create annotation data for gene ratio labels - show every other pathway
n_cytokines <- length(cytokine_order_levels)
gene_ratio_annotations <- pathway_gene_ratio %>%
  mutate(
    pathway_order = factor(pathway_clean, levels = pathway_order_levels),
    label = sprintf("%.2f", sort_gene_ratio),
    y_pos = n_cytokines + 0.7,
    pathway_index = row_number()
  ) %>%
  filter(pathway_index %% 2 == 1)

# SWIMLANE PLOT
swimlane_plot <- ggplot(df, aes(x = pathway_order, y = cytokine_order)) +
  geom_point(aes(size = count, color = p.adjust), alpha = 0.75) +
  
  # Add gene ratio labels at the top (every other pathway)
  geom_text(data = gene_ratio_annotations,
            aes(x = pathway_order, y = y_pos, label = label),
            size = 8/.pt,  # 8 point font
            color = "grey3", angle = 45, hjust = 0, vjust = 0.5,
            inherit.aes = FALSE) +
  
  # Add "Gene Ratio" title
  annotate("text", x = length(pathway_order_levels)/2, 
           y = n_cytokines + 2,
           label = "Gene Ratio", fontface = "bold", 
           size = 10/.pt,  # 10 point font
           color = "grey2") +
  ...
  # Expand x axis on the right side to prevent cutoff
  scale_x_discrete(expand = expansion(add = c(0.5, 1.5))) +
  
  # Expand y axis to make room for gene ratio annotations above
  ...
  theme(
    ...
    # Increased right margin to prevent cutoff
    plot.margin = margin(t = 60, r = 35, b = 35, l = 70),
...
ggsave("gsea_swimlane.eps", swimlane_plot, 
       width = 19.05, height = 14, units = "cm", 
       dpi = 300, device = cairo_ps, bg = "white")

ggsave("gsea_swimlane.pdf", swimlane_plot, 
       width = 19.05, height = 14, units = "cm", device = cairo_pdf)