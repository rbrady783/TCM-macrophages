...
pr <- read.csv("il.10_quartile_results.csv", stringsAsFactors = FALSE) 
...
write.csv(
  ...
  file      = "il.10_GSEA_Hallmark_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "il.10_GSEA_C2_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "il.10_GSEA_C6_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "il.10_GSEA_C7_preRanked.csv",
  row.names = FALSE
)

# 23) Combine all results into master table ----
# Extract cytokine name from filename (adjust as needed)
cytokine_name <- "il.10"  # Change this to your actual cytokine name

collections <- c("C2", "C6", "C7")
gsea_list <- list(gseaRes_c2, gseaRes_c6, gseaRes_c7)
