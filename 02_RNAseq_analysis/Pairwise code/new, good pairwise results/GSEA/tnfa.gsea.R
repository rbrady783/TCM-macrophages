...
pr <- read.csv("tnf.a_quartile_results.csv", stringsAsFactors = FALSE) 
...
write.csv(
  ...
  file      = "tnf.a_GSEA_Hallmark_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "tnf.a_GSEA_C2_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "tnf.a_GSEA_C6_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "tnf.a_GSEA_C7_preRanked.csv",
  row.names = FALSE
)

# 23) Combine all results into master table ----
# Extract cytokine name from filename (adjust as needed)
cytokine_name <- "tnf.a"  # Change this to your actual cytokine name
