...
# =========================================
CYTOKINE <- "TNF.a"  # Change to: VEGF, IL.8, KC.like, IL.10, TNF.a, TGF.b, CCL2
#count_threshold <- 3
#min_samples <- 6
#ycap <- 6
...
# 8) Subset to matched samples only (keeps all genes)
selected_cols <- c(1, which(colnames(raw) %in% groups_df$id))
raw_subset <- raw[, selected_cols]

# Move gene IDs into rownames & drop that column
rownames(raw_subset) <- raw_subset[[1]]
raw_subset <- raw_subset[, -1]

# 9) Build & run DESeq2 ----
dds <- DESeqDataSetFromMatrix(
  countData = raw_subset,  # All genes, matched samples only
  colData   = groups_df[colnames(raw_subset), ],
  design    = ~ group
)
...
# Write results
output_file <- paste0(tolower(CYTOKINE), "_quartile_results_UNFILTERED.csv")
