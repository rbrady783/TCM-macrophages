...
clean_file_name <- function(filename) {
  ...
  cleaned <- str_replace(filename, "_quartile_results", "")
...
csv_files <- list.files(pattern = "(_quartile_results).*\\.(csv|xlsx?)$", full.names = TRUE)
...
# Save the Excel file
output_file <- "Combined_Pairwise_Data.xlsx"
