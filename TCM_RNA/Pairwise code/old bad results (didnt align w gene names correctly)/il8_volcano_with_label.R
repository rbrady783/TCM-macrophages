# =========================================
# il8 High vs. Low: Shrunken Volcano + Highlight
# =========================================
...
il8_groups <- read.csv("il8_groups.csv",     stringsAsFactors = FALSE)

# 2) Subset your il8 samples (adjust cols as needed) ----
raw_il8 <- raw %>%
  dplyr::select(1,3,5,9,16:23,25)
...
write.csv(res_df, "il8_pairwise_filtered.csv", row.names = FALSE)
...
ens.id   <- "ENSCAFG00000003939"  # ← fill in your ENSCAF ID
symbol   <- "NOD1"               # ← fill in the label you want
...
# plot + save
print(p_volcano)
ggsave("volcano_il8_highlight.png",
       p_volcano,
       width  = 7,
       height = 5,
       dpi    = 600,
