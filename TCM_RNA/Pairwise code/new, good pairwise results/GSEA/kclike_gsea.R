...
pr <- read.csv("kc.like_quartile_results.csv", stringsAsFactors = FALSE) 
...
write.csv(
  ...
  file      = "kc.like_GSEA_Hallmark_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "kc.like_GSEA_C2_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "kc.like_GSEA_C6_preRanked.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file      = "kc.like_GSEA_C7_preRanked.csv",
  row.names = FALSE
)

# 23) Combine all results into master table ----
# Extract cytokine name from filename (adjust as needed)
cytokine_name <- "kc.like"  # Change this to your actual cytokine name
